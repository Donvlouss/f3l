...
pub fn root3<T: Float>(factor: [T; 3]) -> [T; 3] {
    ...
    let alpha = -b.powi(3) / T::from(27f32).unwrap() - d / T::from(2f32).unwrap()
        + b * c / T::from(6f32).unwrap();
    ...
    let x2 = front
        + mul_factor
            * ((theta_factor.acos() + T::from(2f32 * std::f32::consts::PI).unwrap())
                / T::from(3f32).unwrap())
            .cos();
    let x3 = front
        + mul_factor
...
pub fn root3_eq2<T: Float>(factor: [T; 3]) -> [T; 3] {
    ...
    let mut out = if a_ == b_ {
        ...
        let top = tri.sqrt();
        let (y1, y2) = (
            (a_ * b + T::from(3f32).unwrap() * (-b_ - top) / T::from(2f32).unwrap())
                .powf(T::from(1. / 3f32).unwrap()),
            (a_ * b + T::from(3f32).unwrap() * (-b_ + top) / T::from(2f32).unwrap())
                .powf(T::from(1. / 3f32).unwrap()),
        );
        [
            (-b - (y1 + y2)) / T::from(3f32).unwrap(),
            (-T::from(2f32).unwrap() * b + (y1 + y2) - T::from(3f32.sqrt()).unwrap() * (y1 - y2))
                / T::from(6f32).unwrap(),
            (-T::from(2f32).unwrap() * b + (y1 + y2) + T::from(3f32.sqrt()).unwrap() * (y1 - y2))
                / T::from(6f32).unwrap(),
        ]
        ...
        [
            -b + k,
            -k * T::from(0.5f32).unwrap(),
            -k * T::from(0.5f32).unwrap(),
        ]
    } else {
        let t = (T::from(2f32).unwrap() * a_ * b - T::from(3f32).unwrap() * b_)
            / (T::from(2f32).unwrap() * a_ * a_.sqrt());
...
fn test_root3_d0() {
    ...
    assert_eq!(
        crate::round_slice_n(answer, 4),
        crate::round_slice_n(target, 4)
    );
}
