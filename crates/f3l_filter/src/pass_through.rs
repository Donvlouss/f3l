    ...
    P:Into<[T; D]> + Clone + Copy,
{
    pub negative: bool,
    pub dim: usize,
    pub bound: Option<(Bound<T>, Bound<T>)>,
    data: Option<&'a Vec<P>>,
    inliers: Vec<usize>,
}
...
where
    P:Into<[T; D]> + Clone + Copy,
{
    pub fn with_data(data: &'a Vec<P>, bound: (Bound<T>, Bound<T>), dim: usize) -> Self {
        Self {
            negative: false,
            dim,
            bound: Some(bound),
    ...
    pub fn set_parameter(&mut self, dim: usize, bound: (Bound<T>, Bound<T>)) {
        self.dim = dim;
    ...
    [T; D]: Into<P>
{
    ...
    fn apply_filter(&mut self) -> bool {
        ...
        let (start, end) = if let Some((start, end)) = self.bound {
            (start, end)
        } else {
            return false;
        };
