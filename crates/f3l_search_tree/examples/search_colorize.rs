...
fn main() {
    ...
    let by = if search_range==0.0 {SearchBy::Count(1)} else {SearchBy::Radius(search_range * search_range)};
    let mut result = TreeRadiusResult::new(search_range * search_range);
    let samples = seeds.iter()
        .map(|&seed| {
            result.clear();
    ...
    let colors = (0..nb_sample)
        .map(|_| random_color().into()).collect::<Vec<Point3::<f32>>>();

    let cloud = vertices.into_iter().zip(cloud_mask)
        .filter(|&(_, m)| m)
        .map(|(v, _)| Point3::new(v.x, v.y, v.z)).collect::<Vec<Point3<f32>>>();
    ...
    while window.render() {
        ...
        cloud.iter()
            .for_each(|v| window.draw_point(v, &w));
        samples.iter().zip(&colors)
            .for_each(|(cluster, c)| {
                cluster.iter()
                    .for_each(|v| window.draw_point(v, c));
            });
        normals.iter()
            .for_each(|(a, b, c, d)| {
